...
# в phrase находится остаток фразы после названия скилла,
...
def play_1(core: VACore, phrase: str):
    ...
    if phrase == "начать" or phrase == "скачать" or phrase == "повторить":
        ...
        core.play_voice_assistant_speech(
            "Число от одного до тридцати загадано. Начинаем!")
...
def play_2(core: VACore, phrase: str):
    ...
    for i in range(1, 31):
        if phrase == num2text(i):
            ...
            if i == questNumber:
                core.say(
                    "Да, ты угадал. Поздравляю с победой! Скажи повторить, если хочешь сыграть еще раз.")
                core.context_set(play_1)
                return
            else:
                txtsay = ""
                ...
                if tries >= 5:
                    txtsay += "Пять попыток прошло, к сожалению, ты проиграл. А я загадала число " + \
                              num2text(questNumber)
